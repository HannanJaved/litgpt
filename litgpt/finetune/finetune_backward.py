...
from litgpt.prompts import save_prompt_style, PromptStyle, AlpacaReverse
...
def setup(
    # Set default checkpoint to the Llama-3.3-70B-Instruct model.
    devices: Union[int, str] = "auto",
    num_nodes: int = 1,
    checkpoint_dir: Path = Path("checkpoints/meta-llama/Llama-3.3-70B-Instruct"),
    ...
    print(f"Finetuning the {checkpoint_dir} model using the LoRA method.")
    # print("Finetuning the Llama-3.3-70B-Instruct model using a reversed instruction dataset.")
    ...
    out_dir = os.path.join(checkpoint_dir, out_dir)
    out_dir = init_out_dir(out_dir)
    print(f"Finetuning the model using the LoRA method with the following settings:")
    ...
    if devices * num_nodes > 1:
        ...
        print(f"Using FSDP strategy with {devices} devices and {num_nodes} nodes.")
    else:
        strategy = "auto"
        print(f"Using single device training with {devices} devices.")
    ...
    if torch.cuda.is_available() and devices > 1:
        ...
        print("NVLink connectivity check passed.")
    print("Setup function is defined.")
...
def fit(
    ...
    while step_count < max_steps:
        ...
        if iter_num % train.log_interval == 0:
            loss_val = running_loss.compute().item()  # expensive device-to-host synchronization
